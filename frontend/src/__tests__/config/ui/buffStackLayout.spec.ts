...
/** Full HD: 1920×950 Viewport, Header 118 + 8 Lücke. */
const FULL_HD = { w: 1920, h: 950, headerBottom: 118 + 8 }
/** Full HD bei 125 %: 1536 CSS-px — die Spur rückt UNTER den Header. */
const FULL_HD_125 = { w: 1536, h: 760, headerBottom: 118 + 8 }

function hudScale(w: number, h: number) {
  return Math.min(1, Math.max(0.52, Math.min(w / 2560, h / 1440)))
}

/** Das Band zwischen Spur-Unterkante und Oberkante des Command-Panels. */
function bandHeight({ w, h, headerBottom }: typeof FULL_HD) {
  const logH = Math.min(
    EVENT_LOG_PANEL_MAX_H,
  ...
  const bottom = BOTTOM_BAR_SIDE_W * hudScale(w, h) + BUFF_STACK_BOTTOM_GAP
  return h - top - bottom
}

describe('buff stack — over the command panel, under the event log', () => {
  ...
  it('fits four rows on Full HD and two compact rows plus pill at 125 % scaling', () => {
    ...
    // Unter 800 px Höhe gilt die Kompaktstufe.
    expect(
      rowsAt(FULL_HD_125, BUFF_STACK_ROW_H_COMPACT, BUFF_STACK_GAP_COMPACT),
    ...
    // Die Pille ersetzt die oberste Zeile — sie muss flacher sein als eine.
