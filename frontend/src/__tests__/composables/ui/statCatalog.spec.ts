...
import { rankEmblemImage } from '@/utils/game/rankEmblem'
...
describe('JOURNEY_KPI_TILES', () => {
  ...
  it('every tile resolves to an existing catalog entry with a unique icon', () => {
    ...
    for (const tile of JOURNEY_KPI_TILES) {
      ...
      if (tile.pair)
        expect(
          cat!.stats.some((s) => s.key === tile.pair),
          `${tile.category}/${tile.pair}`,
        ).toBe(true)
    }
    for (const tile of JOURNEY_KPI_TILES) {
      if (tile.art) expect(CURRENCY_ART[tile.art], `art ${tile.art}`).toBeTruthy()
      if (tile.emblem)
        for (const tier of RANK_TIERS) expect(rankEmblemImage(tier), tier).toBeTruthy()
    }
